...
CITY_ALIASES = {
    ...
    "жовті води": "Жёлтые Воды", "желтые воды": "Жёлтые Воды", "жёлтые воды": "Жёлтые Воды",
    "синельникове": "Синельниково", "синельниково": "Синельниково",
    "самар": "Самар", "новомосковск": "Новомосковск", "новомосковськ": "Новомосковск",
}
...
SYSTEM_PROMPT = """Ты извлекаешь данные из резюме и откликов с сайтов поиска работы.
...
Верни ТОЛЬКО JSON без пояснений, markdown и дополнительного текста:
{
  ...
  "city": "город на русском языке — ТОЛЬКО название города",
  "positions": "должность или название вакансии (на языке оригинала)",
  "source": "Work.ua или Rabota.ua или OLX — определи по логотипу, цветам, стилю сайта. Если не можешь — unknown",
  "experience": "краткое описание опыта работы если есть. Если нет — пустая строка",
  "resume_url": "прямая ссылка на резюме если видна. Если нет — пустая строка"
}

ДВА ТИПА СКРИНШОТОВ — определи какой перед тобой:

ТИП 1 — страница резюме (Work.ua, Rabota.ua):
- ФИО крупно вверху, город в блоке "Місто проживання" / "Город"
- positions = "Розглядає посади" или заголовок резюме
- Ссылка в адресной строке: work.ua/resumes/XXXXX или robota.ua/candidates/XXXXX

ТИП 2 — кабинет работодателя OLX (заголовки "Панель Роботодавця", "Список кандидатів", "Інформація про кандидата"):
- ВАКАНСИЯ = заголовок объявления слева сверху страницы (подчёркнутый крупный текст, например "Бариста в новую кофейню", "Піший Кур'єр 1200 грн день") → это поле positions
- ГОРОД = мелкая подпись СРАЗУ ПОД заголовком вакансии (например "Жовті Води", "Синельникове", "Дніпро, Центральний")
  — если после города указан район через запятую — бери ТОЛЬКО город (Дніпро, Центральний → Днепр)
- ФИО = имя кандидата в центре в блоке "Інформація про кандидата" (НЕ имена из списка слева!)
- ТЕЛЕФОН = номер в правом блоке "Спілкування з кандидатом"
- age почти никогда нет — пустая строка
- source = OLX
...
Нормализация городов на русский: Дніпро/Дніпропетровськ → Днепр, Харків → Харьков, Київ → Киев, Кривий Ріг → Кривой Рог, Запоріжжя → Запорожье, Львів → Львов, Одеса → Одесса, Миколаїв → Николаев, Кам'янське → Каменское, Новомосковськ → Новомосковск, Вінниця → Винница, Івано-Франківськ → Ивано-Франковск, Черкаси → Черкассы, Суми → Сумы, Жовті Води → Жёлтые Воды, Синельникове → Синельниково, Чернігів → Чернигов. Остальные города тоже переводи на русский.
